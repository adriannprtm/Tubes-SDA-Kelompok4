...
void menuIntegral() {
    while (1) {
        int pil;
        const char* hasil;  // Buffer untuk menyimpan hasil
        char input[1024];  // Buffer untuk menyimpan input pengguna
        printf("Menu Integral:\n");
        printf("1. Integral Tentu\n");
        printf("2. Integral Tak Tentu\n");
        printf("Pilih menu: ");
        scanf("%d", &pil);
        getchar();  // Menghilangkan newline dari buffer input
        ...
        switch (pil) {
            case 1:
                ...
                TampilanKalkulator();
                gotoxy(44, 6);
                printf("Masukkan fungsi untuk diintegrasikan (format: IT(ekspresi, x, lower, upper)): ");
                gotoxy(44,7);
                fgets(input, sizeof(input), stdin);
                // Hapus newline di akhir input jika ada
                input[strcspn(input, "\n")] = 0;
                system("cls");
                hasil = calculate_definite_integral(input);
                TampilanKalkulator();
                gotoxy(44,6);
                printf("Hasil Integral Tentu:");
                gotoxy(44,7);
                printf("%s",hasil);
                break;
            case 2:
                ...
                TampilanKalkulator();
                gotoxy(44, 6);
                printf("Masukkan fungsi untuk diintegrasikan (format: ITT(ekspresi, x)): ");
                gotoxy(44, 7);
                fgets(input, sizeof(input), stdin);
                // Hapus newline di akhir input jika ada
                input[strcspn(input, "\n")] = 0;
                system("cls");
                hasil = calculate_indefinite_integral(input);
                TampilanKalkulator();
                gotoxy(44,6);
                printf("Hasil Integral Tak Tentu:");
                gotoxy(44,7);
                printf("%s",hasil);
                break;
            default:
...
void runCalc(){
	...
	scanf("%d",&pil);
    fflush(stdin);
    if((pil == 1 || pil == 2 || pil == 3 || pil == 4|| pil ==5 || pil==0)){
    	char expression[200];
	    switch (pil) {
	    	case 1:
	    		displayexpression:
